...
_FBORPOR(0xFFFF & BORV_20 & PWRT_64 & MCLR_EN & PWMxL_ACT_LO); // Brown Out voltage set to 2v.  Power up time of 64 ms. MCLR is enabled. 
_FGS(CODE_PROT_OFF);  
...
SavedValuesStruct savedValues = {
	...
	413, // 95, //440, //413,								// throttle low voltage (pedal to metal)
	683, // 808, //700, //683,								// throttle high voltage (foot off pedal)
	...
	100,								// max motor amps.  Units in amperes.  400 max I think.
...
volatile unsigned int vRef1 = 0, vRef2 = 0, vRef3 = 0;
...
void InitIORegisters() {
	...
	I_TRIS_CURRENT3	= 1; 	// 1 means configure as input.
	I_TRIS_UV_FAULT = 1; 	// 1 means configure as input.
	I_TRIS_OC_FAULT	= 1; 	// 1 means configure as input.
	I_TRIS_TEMPERATURE = 1; // 1 means configure as input.
	I_TRIS_VALET_MODE = 1; 	// 1 means configure as input.
	I_TRIS_GLOBAL_FAULT	= 1; // 1 means configure as input.
	I_TRIS_DESAT_FAULT = 1; // 1 means configure as input.

	O_TRIS_LED = 0; 			// 0 means configure as output.
	...
	O_TRIS_SOFTWARE_FAULT = 0;	// 0 means configure as output.
	O_LAT_SOFTWARE_FAULT = 0;	// 0 means set a fault.  This guarantees the pwm is disabled.
...
void InitDiscreteADConversions() {
	...
	// AN0 = CH1 = ADCurrentOut
	// AN1 = CH2 = ADVoltageBattery
	// AN2 = CH3 = ADCurrentSolar
	// AN3 = CH0 = ADVoltageSolar
...
void __attribute__ ((__interrupt__,auto_psv)) _ADCInterrupt(void) {
	...
	if ((ADCounter1 & 3) == 0x00) { // 2.5kHz.
		...
		if (ADCurrent1 < 100) { // || ADCurrent2 < 100) { 		// Really, this one should never be much below 512..
		...
		if ((ADCounter2 & 3) == 0) { // only happens every 4 times.  Around 625 Hz.
			...
			throttle_0_16384 = (((unsigned long)tempThrottle) << 14) / (unsigned long)(savedValues.throttleHighVoltage - savedValues.throttleLowVoltage);
...
/*
		...
		*/
		ADCounter2++;
			...
			PDC1 = (((long)pwm_0_4096) * ((long)maxDutyDiv2[x])) >> 11; // >> 11 instead of >> 12 because it's maxDutyDiv2 array, NOT maxDuty array!! So, PDC1 needs an extra *2.
			#else
			PTPER = 999;
			PDC1 = 500;
			#endif
...
void InitCNModule() {
	...
	CNPU1bits.CN6PUE = 0; // Make sure internal pull-up is turned OFF on CN6.
	CNPU1bits.CN7PUE = 0; // Make sure internal pull-up is turned OFF on CN7.
...
void __attribute__((__interrupt__, auto_psv)) _CNInterrupt(void) {
	...
	if (I_PORT_OC_FAULT == 0) {  // It just became 0 like 2Tcy's ago.
		faultBits |= OC_FAULT;
	}
	if (I_PORT_UV_FAULT == 0) {
		faultBits |= UV_FAULT;
	}
}
...
void ClearAllFaults() {
	int i = 0;
	O_LAT_SOFTWARE_FAULT = 1;	// If I bring this low, it causes a hardware fault that latches.
